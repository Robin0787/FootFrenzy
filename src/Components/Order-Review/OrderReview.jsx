...
const OrderReview = () => {
    ...
    const [allProduct, setAllProduct] = useState(orderedProducts);
    ...
    function deleteProduct(id) {
        ...
        const remainingProducts = products.filter(pr => pr._id !== id);
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-reviewOrder m-h-[calc(100vh-60px)] gap-4 lg:gap-10 p-10'>
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 pt-4'>
                {
                    allProduct.map((product) => <SingleProduct key={product._id} product={product} deleteProduct={deleteProduct}/>)
                }
            </div>
            <div>
...
function SingleProduct({product,deleteProduct}) {
    const {img,name,shipping,price,quantity, _id} = product;
    return (
        <div className='flex justify-between items-center gap-4 px-3 py-2 h-[128px] bg-white border-2 border-gray-300 rounded-lg mb-3 font-thin'>
            ...
            <i onClick={() => {deleteProduct(_id)}} className="fa-solid fa-trash-can text-red-400 cursor-pointer bg-red-500 bg-opacity-20 rounded-full p-3"></i>
        </div>
    )
}
